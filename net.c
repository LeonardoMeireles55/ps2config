		...
		*/
		...
		switch (cmdid) {
			...
			case 0x4B:
			{
				...
				cfg->commands[cfg->cmdCount].cmd_4B.Redirect = SWAP_LE(cfg->commands[cfg->cmdCount].cmd_4B.Redirect);
...
int save_NetCfg(const char* filename, const NetCfg_t* cfg) {
	...
	for (uint32_t i = 0; i < cfg->cmdCount; i++) {
        ...
        switch (cfg->commands[i].cmdid) {
			...
			case 0x42:
			{
                if( fwrite(&cfg->commands[cfg->cmdCount].cmd_42.offset, sizeof(uint32_t), 1, file) != 1) {
					perror("Error fwrite cmd_42 offset");
                    return -1;
				}

                if( fwrite(&cfg->commands[cfg->cmdCount].cmd_42.count, sizeof(uint32_t), 1, file) != 1) {
					perror("Error fwrite cmd_42 count");
                    return -1;
				}
				
                if( fwrite(&cfg->commands[cfg->cmdCount].cmd_42.param, sizeof(uint32_t), cfg->commands[cfg->cmdCount].cmd_42.count, file) != cfg->commands[cfg->cmdCount].cmd_42.count) {
					perror("Error fwrite cmd_42 param");
                    return -1;
				}
			...
			case 0x4B:
			{
                if( fwrite(&cfg->commands[cfg->cmdCount].cmd_4B.offset, sizeof(uint32_t), 1, file) != 1) {
					perror("Error fwrite cmd_42 offset");
                    return -1;
				}

                if( fwrite(&cfg->commands[cfg->cmdCount].cmd_4B.Redirect, sizeof(uint32_t), 1, file) != 1) {
					perror("Error fwrite cmd_42 Redirect");
                    return -1;
				}
				cfg->commands[cfg->cmdCount].cmd_4B.Redirect = SWAP_LE(cfg->commands[cfg->cmdCount].cmd_4B.Redirect);
				
				if( fwrite(&cfg->commands[cfg->cmdCount].cmd_4B.data, sizeof(char), 0x10, file) != 0x10) {
					perror("Error fwrite cmd_4B data");
					return -1;
				}
				break;
			}
